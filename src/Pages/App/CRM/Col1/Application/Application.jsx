...
function Application() {
    ...
    const [load, setLoad] = useState({ mService: true, mmoney: true })
    const [open, setOpen] = useState({ mService: false, mmoney: false })
    ...
    return (
        <>
            <Grid container>
                <Grid item xs={12} container className='link-box-pointer' onClick={() => setOpen({ ...open, mService: true })}>
                    ...
                    <Grid item xs={4}>
                        <div className='right' style={{ paddingTop: 3 }}>
                            {mService ? <CheckCircle className="success" checked={mService} /> : <Cancel className="danger" checked={mService} />}
                        </div>
                    </Grid>
                    <Grid item xs={2} style={{ paddingTop: 3 }}>
                        {load.mService ? <Skeleton animation="wave" /> : <div className={count.mService > 0 ? 'text-right bage-success' : 'text-right bage-error'}><u>{count?.mService}</u></div>}
                    </Grid>
                </Grid>
                ...
                <Grid item xs={12} container className='link-box-pointer' onClick={() => setOpen({ ...open, mmoney: true })}>
