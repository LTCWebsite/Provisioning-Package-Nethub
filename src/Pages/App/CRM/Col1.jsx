...
function Col1() {
  ...
  useEffect(() => {
    ...
    AxiosReq.get("QueryHLRInfo?msisdn=" + phone, {
      ...
      .then((res) => {
        if (res.status === 200) {
          ...
          setCheck({
            ...
            ir_call:
              res.data?.queryServiceResult?.irCall === "1" ? true : false,
            ir_data:
              res.data?.queryServiceResult?.irData === "1" ? true : false,
            load: false,
          });
        }
      })
  ...
  useEffect(() => {
    let phone = localStorage.getItem("ONE_PHONE");
    AxiosReq.get("api/Network5g?Msisdn=" + phone, {
      headers: { Authorization: "Bearer " + cookie.get("ONE_TOKEN") },
    })
      .then((res) => {
        if (res.status === 200) {
      ...
      <Grid container>
        ...
        >
          <Grid
            ...
            xs={12}
            md={12}
            lg={`${type?.NETWORK_CODE === "F" ? 12 : 12}`}
          >
            <Grid item xs={12}>
              <h2 className="blue">Info</h2>
            </Grid>
            {type?.NETWORK_CODE === "F" ? (
            ...
            type?.NETWORK_CODE === "P" ||
            type?.NETWORK_CODE === "F" ||
            type?.NETWORK_CODE === "WP" ||
            type?.NETWORK_CODE === "L" ||
            type?.NETWORK_CODE === "DR" ? (
              <Debit />
            ) : null}
          </Grid>
