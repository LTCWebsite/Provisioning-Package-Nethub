...
function BuyPackage({ open, cb, done, ifdone, count }) {
  ...
  const columns = [
    ...
    { title: "ຈຳນວນດາຕ້າ", field: "pK_DESC", minWidth: 100, sorting: false },
    {
      ...
      field: "pK_CHG",
      minWidth: 100,
      render: (row) => row?.pK_CHG.toLocaleString(),
    },
  ...
  useEffect(() => {
    ...
    AxiosReq.get("Package?networkType=" + network?.NETWORK_CODE, {
  ...
  return (
    <>
      <Dialog open={open} onClose={() => cb(!open)} maxWidth={1200}>
        <Grid container>
          <Grid item xs={3}></Grid>
          <Grid item xs={6}>
          ...
          <Grid item container xs={12} style={{ width: 600 }}>
            ...
            <Grid item xs={1}>
              <div className="right">
                <IconButton
                  aria-label="delete"
                  onClick={() => setIsShowSuccess(!isShowSuccess)}
                >
                  <Close />
                </IconButton>
              </div>
