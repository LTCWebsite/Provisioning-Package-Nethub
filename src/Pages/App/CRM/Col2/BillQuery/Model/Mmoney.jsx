...
function Mmoney({ open, cb, data }) {
    ...
    const columns = [
        ...
        { title: 'ຈຳນວນເງີນ', field: 'adjustAmount', type: 'numeric', render: row => row.adjustAmount.toLocaleString() },
        { title: 'ກ່ອນການເຕີມ', field: 'CUR_BALANCE', render: row => row?.CUR_BALANCE?.toLocaleString() },
        { title: 'ຫຼັງການເຕີມ', field: 'CUR_BALANCE', type: 'numeric', render: row => (row.CUR_BALANCE + row.adjustAmount).toLocaleString() },
    ]
        ...
        <>
            ...
            >
                <Grid container>
                    <Grid item xs={3}></Grid>
                    <Grid item xs={6}>
                        <DialogTitle className='center'>Mmoney Log</DialogTitle>
                    </Grid>
                    <Grid item xs={3}>
                ...
                <Grid container>
                    <Grid item xs={12} style={{ width: 1400 }}>
                        <MyTable tTitle={"Mmoney Log"} tData={data} tColumns={columns} />
                    </Grid>
                </Grid>
            </Dialog>
        </>
