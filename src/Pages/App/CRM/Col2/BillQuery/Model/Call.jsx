...
function Call({ open, cb, data }) {
    ...
    const columns = [
        ...
        { title: 'ກ່ອນຕັດເງີນ', field: 'CurrentAmount' , type: 'numeric', render: row => row.CurrentAmount == null ? 0 :(parseInt(row.CurrentAmount)  + row.Charge).toLocaleString() },
        { title: 'ຫຼັງຕັດເງີນ', field: 'CurrentAmount', type: 'numeric', render: row => row.CurrentAmount == null ? 0 :(parseInt(row.CurrentAmount))?.toLocaleString() },
