...
function All({ open, cb, data }) {
    // console.log(data)
    const columns = [
        { title: 'Transaction ID', field: 'Ext_transID', render: row => row?.Ext_transID },
        { title: 'ເບີຕົ້ນທາງ', field: 'msisdn', render: row => row.msisdn === undefined ? row.CallingPartyNumber === undefined ? row?.PRI_IDENTITY : row.CallingPartyNumber : row.msisdn },
        { title: 'ເບີປາຍທາງ', field: 'CalledPartyNumber', render: row => row?.CalledPartyNumber === undefined ? null : row?.CalledPartyNumber?.substr(0, 7) + "xxx" + row?.CalledPartyNumber?.substr(10) },
        ...
        {
            title: 'ປະເພດ', field: 'type', render: row => <u style={{
                ...
                color: '#fff',
                padding: '2px 10px'
            }}>{row.type==="happyCall"  ? "Debug Game" : row.type}</u>
        },
