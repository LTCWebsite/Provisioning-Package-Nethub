  ...
  return (
    <React.Fragment>
      <TableRow sx={{ "& > *": { borderBottom: "unset" } }}>
        ...
        <TableCell>
          {String(row.LogDate).replace(/(\d{4})(\d{2})(\d{2})/g, "$1-$2-$3")}
        </TableCell>
      </TableRow>
      <TableRow>
          ...
          <Collapse in={open} timeout="auto" unmountOnExit>
            <Box sx={{ margin: 1 }}>
              ...
              <Table size="small" aria-label="purchases">
                ...
                <TableBody>
                  {row.Tbllogs &&
                    row.Tbllogs.map((historyRow, index) => (
                      <TableRow key={index}>
                        <TableCell>{historyRow.Msisdn}</TableCell>
                        <TableCell>
                          {historyRow.PackageAmount?.toLocaleString()}
                        </TableCell>
                        <TableCell>
                          {historyRow.PaymentAmount?.toLocaleString()}
                        </TableCell>
                ...
                <TableBody>
                  {rows.map((row, index) => (
                    <Row
                      key={index}
                      row={row}
                      allMsisdns={allMsisdns}
                      onRefreshFtth={onRefreshFtth}
                    />
                  ))}
                </TableBody>
              </Table>
