        ...
        <>
            <Grid container>
                <Grid item xs={12} container className='link-box-pointer' onClick={handleOpen}>
                    <Grid item xs={2}><SimCardIcon /></Grid>
                    <Grid item xs={6}>ເບີແຖມຟຣີ</Grid>
                    <Grid item xs={4}>
                                    ...
                                    {bookingRows.map((row, index) => (
                                        <TableRow key={row.Id || index}>
                                            ...
                                            <TableCell>
                                                <button
                                                    style={{
                                                        backgroundColor: !row.bookStatus === true ? '#ccc' : '#4CAF50',
                                                        color: !row.bookStatus === true ? '#666' : 'white',
                                                        padding: '5px 15px',
                                                        border: 'none',
                                                        borderRadius: '4px',
                                                        cursor: !row.bookStatus === true ? 'not-allowed' : 'pointer',
                                                        fontSize: '12px'
                                                    }}
                                                    disabled={!row.bookStatus === true}
                            ...
                            <Grid container spacing={1}>
                                {rows.filter(item => item.Msisdn && item.Msisdn.trim() !== '').map((msisdn, index) => (
                                    <Grid item xs={6} sm={4} md={3} lg={2.4} xl={2} key={msisdn.ID || index}>
                                        <Box
                                            onClick={() => handleChooseMsisdn(msisdn)}
                                            sx={{
                                                display: 'flex',
                                                alignItems: 'center',
