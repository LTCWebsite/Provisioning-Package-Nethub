...
function Info178({ open, cb }) {
  ...
  const columns = [
    { title: "No", field: "idx", maxWidth: 50 },
    {
      ...
      minWidth: 400,
      render: (row) =>
        row.nameL2 !== "" ? row.nameL1 + " - " + row.nameL2 : row.nameL1,
    },
                  ...
                  >
                    <Grid item container xs={10} spacing={3}>
                      ...
                      <Grid item xs={4} style={{ paddingLeft: 17 }}>
                        <input
                          className="input"
                          placeholder="ຄົ້ນຫາສະຖານທີ່ຕ່າງໆ..."
                          onChange={(e) => {
                      ...
                      <Grid item xs={4} style={{ paddingLeft: 25 }}>
                        <Select
                          ...
                          defaultValue={0}
                          onChange={(e) =>
                            setSl({ ...sl, select: e.target.value })
                          }
                        >
                          <MenuItem value={0} disabled className="option">
                            ເລືອກແຂວງ
                          </MenuItem>
                          {sl.data?.map((row) => {
                            return (
                              <MenuItem value={row.codeKey}>
                                {row?.codeDesc}
                              </MenuItem>
                            );
                          })}
                    ...
                    <Grid
                      item
                      xs={2}
                      style={{ paddingLeft: 35, paddingBottom: 20 }}
                    >
                      <Button
                        fullWidth
                        variant="contained"
