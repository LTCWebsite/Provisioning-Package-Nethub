...
export default function AdditionalEditDialog({ open, onClose, onSuccess, data }) {
  ...
  useEffect(() => {
    if (data && open) {
      setFormData({
        ...
        refillStopDay: data.refillStopDay || '',
        sms: data.sms || '',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await AxiosReq3.put(`/AdditionalNethub/${formData.id}`, payload, {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 2, bgcolor: '#f5f5f5' }}>
        <Typography variant="h6" sx={{ fontWeight: 'bold' }}>ແກ້ໄຂ Additional</Typography>
              ...
              <FormControl fullWidth size="small">
                <Select
                  ...
                  renderValue={(selected) => {
                    if (!selected) return "ເລືອກ Id Package";
