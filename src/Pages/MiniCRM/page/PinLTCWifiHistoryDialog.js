...
export default function PinLTCWifiHistoryDialog() {
    ...
    const CodaPayFilter = () => {
        ...
        Axios.post("HistoryBuyPackagePinLtcWifi", sendData, { headers: { 'Authorization': 'Bearer ' + cookie.get("one_session") } }).then(res => {
            if (res.status === 200) {
                var num = 0
    ...
    function ShowData() {
        return (
            <>
                <MyTable tTitle={"Pin LTC Wifi History Detail [ " + moment(startDate).format("DD-MM-YYYY") + ' - ' + moment(endDate).format("DD-MM-YYYY") + " ]"} tData={data} tColumns={columns} />
            </>
        )
    }
                ...
                <AppBar className="dialog-header">
                    <Toolbar>
                        ...
                        <Typography variant="h6" className={classes.title}>
                            Pin LTC Wifi History Detail ( {phone} )
                        </Typography>
                    </Toolbar>
                </AppBar>
                <Grid container>
