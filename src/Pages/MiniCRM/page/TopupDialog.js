...
//         Axios.post("controller", sendData, { headers: { 'Authorization': 'Bearer ' + cookie.get("one_session") } }).then(res => {
        Axios.post("controller?Telephone=" + phone + "&msisdn=" + phone, sendData, { headers: { 'Authorization': 'Bearer ' + cookie.get("one_session") } }).then(res => {
            if (res.status === 200) {
                var num = 0
    ...
    function ShowData() {
        return (
            <>
                <MyTable tTitle={"Topup and M-Topup Detail [ " + moment(startDate).format("DD-MM-YYYY") + ' - ' + moment(endDate).format("DD-MM-YYYY") + " ]"} tData={data} tColumns={columns} />
            </>
        )
    }
                ...
                <AppBar className="dialog-header">
                    <Toolbar>
                        ...
                        <Typography variant="h6" className={classes.title}>
                            Topup and M-Topup Detail ( {phone} )
                        </Typography>
                    </Toolbar>
                </AppBar>
                <Grid container>
