...
export default function BSSdialog() {
    ...
    const CodaPayFilter = () => {
        ...
        Axios.post("QueryBSS", sendData, { headers: { 'Authorization': 'Bearer ' + cookie.get("one_session") } }).then(res => {
            if (res.status === 200) {
                var num = 0
    ...
    const columns = [
        ...
        { title: 'Month Bill', field: 'date' },
        { title: 'Chanel', field: 'staffcode' },
        { title: 'Monthly Fee', field: 'monthly_Fee', type: 'numeric', render: row => row.monthly_Fee > 0 ? parseInt(row.monthly_Fee).toLocaleString() : row.monthly_Fee },
        { title: 'OverPaid Amount', field: 'overPaid_Amount', type: 'numeric', render: row => row.overPaid_Amount > 0 ? parseInt(row.overPaid_Amount).toLocaleString() : row.overPaid_Amount  },
        { title: 'owe', field: 'owe', type: 'numeric', render: row => row.owe > 0 ? parseInt(row.owe).toLocaleString() : row.owe  },
        { title: 'Payment', field: 'payment', type: 'numeric', render: row => row.payment > 0 ? parseInt(row.payment).toLocaleString() : row.payment  },
    ]
    function ShowData() {
        return (
            <>
                <MyTable tTitle={"BSS Detail [ " + moment(startDate).format("DD-MM-YYYY") + ' - ' + moment(endDate).format("DD-MM-YYYY") + " ]"} tData={data} tColumns={columns} />
            </>
        )
    }
