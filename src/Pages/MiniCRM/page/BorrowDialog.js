...
export default function BorrowDialog() {
    ...
    const CodaPayFilter = () => {
        ...
        Axios.post("BorrowfadaoAndKalsym?msisdn=" + phone + "&start_date=" + date_start + "&en_date=" + date_end, sendData, { headers: { 'Authorization': 'Bearer ' + cookie.get("one_session") } }).then(res => {
            if (res.status === 200) {
                var num = 0
    ...
    const columns = [
        ...
        // { title: 'Borrow ID', field: 'borrowId' },
        ...
        { title: 'ຕ້ອງຈ່າຍ', field: 'paid', type: 'numeric', render: row => row.paid > 0 ? row.paid.toLocaleString() : <u className="red">{row.paid}</u> },
        { title: 'ສະຖານະ', field: 'resultDesc' },
    ]
    function ShowData() {
        return (
            <>
                <MyTable tTitle={"Borrow Detail [ " + moment(startDate).format("DD-MM-YYYY") + ' - ' + moment(endDate).format("DD-MM-YYYY") + " ]"} tData={data} tColumns={columns} />
            </>
        )
    }
