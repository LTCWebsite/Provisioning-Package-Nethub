...
export default function GameLoftDialog() {
    ...
    const CodaPayFilter = () => {
        ...
        Axios.post("GameLoft", sendData, { headers: { 'Authorization': 'Bearer ' + cookie.get("one_session") } }).then(res => {
            if (res.status === 200) {
                var num = 0
    ...
    function ShowData() {
        return (
            <>
                <MyTable tTitle={"GameLoft Detail [ " + moment(startDate).format("DD-MM-YYYY") + ' - ' + moment(endDate).format("DD-MM-YYYY") + " ]"} tData={data} tColumns={columns} />
            </>
        )
    }
